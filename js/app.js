...
import { filterToEvent, sortToEvent } from "./sort_and_filter.js";

const inputText = document.getElementById("search-text");
inputText.addEventListener("input", filterToEvent);

const allTitles = document.querySelectorAll(".title[data-name]");
for (let element of allTitles) {
    element.addEventListener("click", sortToEvent);
}

export async function appendAllStudents(allStudents) {
    const tbody = document.querySelector("tbody");
    tbody.innerHTML = "";

    for (let StudentObj of allStudents) {
